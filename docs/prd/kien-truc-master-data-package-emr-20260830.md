...
- **Phiên bản**: 1.0 (Draft — chờ BO review)
- **Ngày**: 2026-08-30
...
| **Q3.2** | Chốt **bộ cột nguồn sự thật** cho thuốc (`9005` hay `9010`)? Đề xuất `9005`. Có được phép sửa `DrugHandlers.cs` để bỏ cột legacy không? |
...
### Phương án A — Trường tham chiếu tối giản trên `pkg_subscriptions` ✅ **ĐỀ XUẤT CHỌN**
...
| # | Rủi ro | Mức | Giảm thiểu |
|---|---|---|---|
| R4.1 | `external_pathway_stage` cập nhật tay → **dữ liệu cũ/sai hiển thị cho bác sĩ**, có thể ảnh hưởng quyết định lâm sàng | 🟠 TB | Bắt buộc hiển thị `external_synced_at` cạnh stage + badge đỏ *"Dữ liệu tham chiếu, cập nhật … ngày trước"*. **Không** dùng field này cho bất kỳ logic tự động nào |
...
## 4.6 Câu hỏi mở — Mục 4

| ID | Câu hỏi |
|---|---|
| **Q4.1** | **(Quan trọng nhất)** Bệnh nhân có gói **walk-in không qua đặt lịch** thì trừ định mức lượt khám ở đâu? Mọi Encounter đều bắt buộc qua check-in tiếp đón (⇒ đã đủ), hay cần thêm điểm trừ tại Encounter? |
| **Q4.2** | Ai cập nhật `external_pathway_stage` trong MVP — lễ tân, điều phối viên diaB, hay import CSV định kỳ? Tần suất? |
| **Q4.3** | Nhãn giai đoạn muốn hiển thị dạng nào: **"Tuần 6/24"** (theo đúng diaB) hay tên giai đoạn nghiệp vụ (**"Khởi động / Ổn định / Duy trì"**)? Nếu là tên nghiệp vụ thì ai định nghĩa bảng ánh xạ? |
| **Q4.4** | 1 `pkg_subscription` của HIS có tương ứng **1-1** với 1 gói diaB không? (diaB ràng buộc 1 account = 1 gói active; HIS cho nhiều). Nếu 1 gói diaB tương ứng nhiều subscription HIS → `external_subscription_ref` không unique được. |
| **Q4.5** | Có cần lưu **lịch sử** thay đổi giai đoạn (append-only) hay chỉ cần giá trị hiện tại? Đề xuất: chỉ hiện tại (MVP). |
...
| ID | Câu hỏi |
|---|---|
| **Q5.1** | **(Quan trọng nhất)** Có gộp `diab_his_cli_diabetes_templates` vào `EmrTemplate` không? Nếu không gộp thì **không nên** làm `schema_json` (sẽ thành 3 luồng). |
...
|---|---|---|---|
| 1 | Trả lời **Q4.1** (walk-in không trừ VISIT) — nguy cơ thất thoát doanh thu | — | 🔴 Ngay |
| 2 | Trả lời **Q3.6** (có BHYT không) — quyết định ~10 cột `bhyt_*` có làm hay không | — | 🔴 Ngay |
| 3 | Trả lời **Q5.1** (gộp `diabetes_templates`) — quyết định có làm `schema_json` không | — | 🔴 Ngay |
| 4 | Migration `9183` (external pathway ref) + UI hiển thị — **rẻ, giá trị ngay** | Q4.2, Q4.3 | 🟠 Cao |
| 5 | Migration `9184` (`encounter.covered_by_subscription_id`) — nền cho Mục 5 | — | 🟠 Cao |
| 6 | Migration `9180`/`9181` (master data) — chia 2 đợt: đợt 1 non-BHYT, đợt 2 BHYT | Q3.1, Q3.2, Q3.6 | 🟠 Cao |
| 7 | Migration `9185` (template ↔ package map) — **bỏ phần `schema_json`** nếu Q5.1 = không gộp | Q5.1, Q5.4 | 🟡 TB |
| 8 | Dọn nợ cột trùng lặp `9005` vs `9010` (migration riêng, sau khi sửa `DrugHandlers.cs`) | Q3.2 | 🟡 TB |
| 9 | ADR `docs/adr/0012-tham-chieu-lo-trinh-diab.md` ghi trade-off PA A vs PA B | — | 🟡 TB |
