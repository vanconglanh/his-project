...
| F7 — Report endpoint stub | ✅ Đã sửa (2026-07-02) | 8/8 endpoint có query thật (Dapper + tenant filter), +2 calculator thuần + 9 unit test; 505 unit test PASS. Lưu ý: chưa chạy integration test MySQL thật (môi trường thiếu Docker) |
| F8 — Hex ngoài chart (avatar, letterhead) | ✅ Đã sửa (2026-07-02) | Token `--avatar-1..10` (+foreground) và `--print-header`/`--print-header-foreground` trong globals.css |
...
**Tồn đọng chuyển đợt sau:** L-06/F-01 (gộp luồng tạo bệnh nhân ở Tiếp đón) và F-03 (PO/GRN/Adjustment → Fullpage) — **Lành đã thẩm định, ĐỒNG Ý CÓ ĐIỀU CHỈNH**, kế hoạch chi tiết tại `docs/review/architect-decision-f01-f03.md`, chờ Đăng (PO) chốt 3 câu hỏi mở trước khi triển khai; F-09 (phổ cập `StickyActionBar`/`FieldGroup`, dựng `FullPageFormShell`); PageHeader component chống lệch title về sau. ~~F7/F8~~ đã hoàn tất 2026-07-02 (xem bảng trạng thái).

---

## Đợt 3 — Triển khai F-01/F-03 + tồn đọng (2026-07-03)

PO đã chốt 3 câu hỏi mở → triển khai theo kế hoạch Lành duyệt:
- **F-01 ✅** — Xóa `PatientForm.tsx` (bản sao thiếu 6 field, rủi ro toàn vẹn dữ liệu); Tiếp đón gộp về `/patients/new?returnTo=/reception`, nối vòng **auto-select** bệnh nhân khi quay lại, **lưu draft check-in** (sessionStorage, khôi phục âm thầm — theo yêu cầu PO "không mất dữ liệu, dễ dùng").
- **F-03 ✅** — 3 form kho dược có sub-list chuyển sang Fullpage: `pharmacy/purchase-orders/new`, `pharmacy/adjustments/new`, `pharmacy/grn/new?poId=` (GRN mang context PO — PO yêu cầu "nhập nhiều item một lúc"). Khai sinh `FullPageFormShell` dùng chung (tái dùng `StickyActionBar`) → chặn nợ F-09; dọn nợ dual-dialog Adjustment.
- **F-09 🟡 một phần** — `FullPageFormShell` đã có và dùng ở 3 route mới; còn retrofit 3 form Fullpage cũ + phổ cập `FieldGroup`.
- **PRD Drug Picker** — Đăng viết `docs/prd/drug-picker-prd.md`: nâng cấp `DrugAutocomplete` có sẵn (thêm điều hướng bàn phím batch, tồn kho, lọc INACTIVE) thay vì viết mới. Cần BE xác nhận 2 điểm (search theo hoạt chất; xử lý thuốc ngừng KD) trước khi triển khai. **Chưa code.**

**Tồn đọng còn lại:** triển khai Drug Picker (sau khi BE xác nhận); retrofit F-09 cho 3 form cũ; `PageHeader` chống lệch title; audit a11y sâu (đo contrast, tab-order thủ công, tablet thật); kiểm chứng runtime (report endpoints trên MySQL thật, in biên lai sau sửa F4, xem light mode bằng mắt).
