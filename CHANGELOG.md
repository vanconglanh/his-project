...
- **Seed E2E `db/seeds/e2e_billing_finalized.sql`:** `id` encounter không phải GUID hex (`e2eenc01-…`, chữ `n`) làm EF Core `Guid.TryParse` ném `FormatException` khi materialize → **`GET /api/v1/encounters` trả 500** (sập cả danh sách lượt khám). Sửa id thành hex hợp lệ. *Phát hiện khi chạy E2E trên stack thật (WSL MySQL 8.4) — xem `docs/test/live-e2e-2026-07-03.md`.*
